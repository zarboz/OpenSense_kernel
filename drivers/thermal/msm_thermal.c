...
//Sampling interval
#define DEF_THERMAL_CHECK_MS 500
	...
	for_each_possible_cpu(cpu) {
		...
		//low clr point
			...
			update_policy = true;
                        ...
                        if (cpu == (CONFIG_NR_CPUS-1)) {
                                thermal_throttled = 0;
                                pr_warn("msm_thermal: Low thermal throttle ended! temp:%lu\n", temp);
                        }
		//mid trip point
		...
		//mid clr point
			...
			update_policy = true;
                        if (cpu == (CONFIG_NR_CPUS-1)) {
                                thermal_throttled = 1;
                                pr_warn("msm_thermal: Mid thermal throttle ended! temp:%lu\n", temp);
                        }
		//max trip point
		...
		//max clr point
			...
			update_policy = true;
                        if (cpu == (CONFIG_NR_CPUS-1)) {
                                thermal_throttled = 2;
                                pr_warn("msm_thermal: Max thermal throttle ended! temp:%lu\n", temp);
                        }
		}
